...
<body>
	...
	<div class="text">
		<div class="news">
			<c:forEach items="${newsList}" begin="0" end="5" var="news"
				varStatus="row">
				<div>${row.index + 1 }&emsp;</div>
				...
				<div>
					<span>&emsp;&emsp;&emsp; ${news.create_time }</span>
				</div>
				<div style="float: right">
					<td><a href="DeleteNews?news_id=${news.news_id}"
		...
		<c:forEach begin="1" end="${pageNum }" var="pageIndex">
			<a href="pageServlet?currpage=${pageIndex }&pagesiaze=6&manage=1"
				target="_main" id="a">${pageIndex }</a>
		</c:forEach>
	</div>
